...
public class Program
{
    ...
    [DllImport("user32.dll")] static extern uint SendInput(uint cInputs, INPUT[] pInputs, int cbSize);
    ...
    [DllImport("user32.dll", SetLastError = true)] [return: MarshalAs(UnmanagedType.Bool)] static extern bool UnhookWindowsHookEx(IntPtr hhk);
    ...
    public static void Run(string text)
    {
        ...
        if (_hookId == IntPtr.Zero) { Console.WriteLine("Hook failed"); return; }
        Console.WriteLine("Waiting for RShift...");
        while (!_exitRequested) { if (GetMessage(out MSG msg, IntPtr.Zero, 0, 0) > 0) { } }
    ...
    static void PressKey(ushort vk)
    {
        ...
        inputs[0] = MakeKeyInput(vk, false); inputs[1] = MakeKeyInput(vk, true);
        SendInput(2, inputs, INPUT.Size);
    }
    static void PressKeyWithModifier(ushort vk, ushort mod)
    {
        INPUT[] inputs = new INPUT[4];
        inputs[0] = MakeKeyInput(mod, false); inputs[1] = MakeKeyInput(vk, false);
        inputs[2] = MakeKeyInput(vk, true); inputs[3] = MakeKeyInput(mod, true);
    ...
    static void SendUnicodeString(string text)
    {
        ...
        {
            INPUT[] inputs = new INPUT[2];
            inputs[0].type = 1; inputs[0].U.ki.wVk = 0; inputs[0].U.ki.wScan = c; inputs[0].U.ki.dwFlags = 0x0004; inputs[0].U.ki.time = 0; inputs[0].U.ki.dwExtraInfo = IntPtr.Zero;
            inputs[1].type = 1; inputs[1].U.ki.wVk = 0; inputs[1].U.ki.wScan = c; inputs[1].U.ki.dwFlags = 0x0004 | 0x0002; inputs[1].U.ki.time = 0; inputs[1].U.ki.dwExtraInfo = IntPtr.Zero;
            SendInput(2, inputs, INPUT.Size); Thread.Sleep(1);
        }
    }
    static INPUT MakeKeyInput(ushort vk, bool up)
    {
        INPUT input = new INPUT { type = 1, U = new InputUnion { ki = new KEYBDINPUT { wVk = vk, wScan = 0, dwFlags = up ? 0x0002u : 0u, time = 0, dwExtraInfo = IntPtr.Zero } } };
        return input;
    }
}
'@
